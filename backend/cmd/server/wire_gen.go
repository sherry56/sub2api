...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	researchDrawingImage2RecordRepository := repository.NewResearchDrawingImage2RecordRepository(db)
	researchDrawingImage2RecordService := service.NewResearchDrawingImage2RecordService(researchDrawingImage2RecordRepository)
	researchDrawingHandler := handler.NewResearchDrawingHandler(userService, settingService, researchDrawingImage2RecordService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, researchDrawingHandler, idempotencyCoordinator, idempotencyCleanupService)
