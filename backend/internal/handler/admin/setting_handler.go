...
func (h *SettingHandler) GetSettings(c *gin.Context) {
	...
	payload := dto.SystemSettings{
		...
		ResearchDrawingExpMode:                          settings.ResearchDrawingExpMode,
		ResearchDrawingRetrievalSetting:                 settings.ResearchDrawingRetrievalSetting,
		ResearchDrawingNumCandidates:                    settings.ResearchDrawingNumCandidates,
		ResearchDrawingAspectRatio:                      settings.ResearchDrawingAspectRatio,
...
type UpdateSettingsRequest struct {
	...
	// Research Drawing Pipeline (PaperBanana settings)
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	// Research Drawing pipeline 参数规范化（兼容旧客户端未传字段）
	const (
	...
	switch req.ResearchDrawingExpMode {
	case "demo_full", "demo_planner_critic":
	default:
		if strings.TrimSpace(previousSettings.ResearchDrawingExpMode) != "" {
			req.ResearchDrawingExpMode = strings.TrimSpace(previousSettings.ResearchDrawingExpMode)
		} else {
			req.ResearchDrawingExpMode = researchDrawingDefaultExpMode
		}
	}
	req.ResearchDrawingRetrievalSetting = strings.TrimSpace(req.ResearchDrawingRetrievalSetting)
	switch req.ResearchDrawingRetrievalSetting {
	case "auto", "manual", "random", "none":
	default:
		if strings.TrimSpace(previousSettings.ResearchDrawingRetrievalSetting) != "" {
			req.ResearchDrawingRetrievalSetting = strings.TrimSpace(previousSettings.ResearchDrawingRetrievalSetting)
		} else {
			req.ResearchDrawingRetrievalSetting = researchDrawingDefaultRetrievalSetting
		}
	}
	req.ResearchDrawingAspectRatio = strings.TrimSpace(req.ResearchDrawingAspectRatio)
	switch req.ResearchDrawingAspectRatio {
	case "21:9", "16:9", "3:2":
	default:
		if strings.TrimSpace(previousSettings.ResearchDrawingAspectRatio) != "" {
			req.ResearchDrawingAspectRatio = strings.TrimSpace(previousSettings.ResearchDrawingAspectRatio)
		} else {
			req.ResearchDrawingAspectRatio = researchDrawingDefaultAspectRatio
		}
	}
	if req.ResearchDrawingNumCandidates < 1 || req.ResearchDrawingNumCandidates > 20 {
		if previousSettings.ResearchDrawingNumCandidates >= 1 && previousSettings.ResearchDrawingNumCandidates <= 20 {
			req.ResearchDrawingNumCandidates = previousSettings.ResearchDrawingNumCandidates
		} else {
			req.ResearchDrawingNumCandidates = 1
		}
	}
	if req.ResearchDrawingMaxCriticRounds < 1 || req.ResearchDrawingMaxCriticRounds > 5 {
		if previousSettings.ResearchDrawingMaxCriticRounds >= 1 && previousSettings.ResearchDrawingMaxCriticRounds <= 5 {
			req.ResearchDrawingMaxCriticRounds = previousSettings.ResearchDrawingMaxCriticRounds
		} else {
			req.ResearchDrawingMaxCriticRounds = 2
		}
	}
	req.ResearchDrawingMainModelName = strings.TrimSpace(req.ResearchDrawingMainModelName)
	if req.ResearchDrawingMainModelName == "" {
		req.ResearchDrawingMainModelName = strings.TrimSpace(previousSettings.ResearchDrawingMainModelName)
	}
	if req.ResearchDrawingMainModelName == "" {
		req.ResearchDrawingMainModelName = researchDrawingDefaultMainModelName
	}
	if len(req.ResearchDrawingMainModelName) > researchDrawingModelNameMaxLen {
		req.ResearchDrawingMainModelName = req.ResearchDrawingMainModelName[:researchDrawingModelNameMaxLen]
	}
	...
	req.ResearchDrawingImageGenModelName = strings.TrimSpace(req.ResearchDrawingImageGenModelName)
	if req.ResearchDrawingImageGenModelName == "" {
		req.ResearchDrawingImageGenModelName = strings.TrimSpace(previousSettings.ResearchDrawingImageGenModelName)
	}
	if req.ResearchDrawingImageGenModelName == "" {
		req.ResearchDrawingImageGenModelName = researchDrawingDefaultImageGenModelName
	}
	if len(req.ResearchDrawingImageGenModelName) > researchDrawingModelNameMaxLen {
	...
	if req.ResearchDrawingMaxRefineResolution != "2K" && req.ResearchDrawingMaxRefineResolution != "4K" {
		prevRefine := strings.ToUpper(strings.TrimSpace(previousSettings.ResearchDrawingMaxRefineResolution))
		if prevRefine == "2K" || prevRefine == "4K" {
			req.ResearchDrawingMaxRefineResolution = prevRefine
		} else {
			req.ResearchDrawingMaxRefineResolution = researchDrawingDefaultMaxRefineResolution
		}
	}
	researchDrawingUnitPrice := previousSettings.ResearchDrawingUnitPrice
	if req.ResearchDrawingUnitPrice != nil && *req.ResearchDrawingUnitPrice > 0 {
		researchDrawingUnitPrice = *req.ResearchDrawingUnitPrice
	}
	if researchDrawingUnitPrice <= 0 {
		researchDrawingUnitPrice = 2.99
	}
	researchDrawingMethodOptimizationEnabled := previousSettings.ResearchDrawingMethodOptimizationEnabled
	if req.ResearchDrawingMethodOptimizationEnabled != nil {
		researchDrawingMethodOptimizationEnabled = *req.ResearchDrawingMethodOptimizationEnabled
	}
	researchDrawingMethodOptimizationDefaultEnabled := previousSettings.ResearchDrawingMethodOptimizationDefaultEnabled
	if req.ResearchDrawingMethodOptimizationDefaultEnabled != nil {
		researchDrawingMethodOptimizationDefaultEnabled = *req.ResearchDrawingMethodOptimizationDefaultEnabled
	}
	if !researchDrawingMethodOptimizationEnabled {
		researchDrawingMethodOptimizationDefaultEnabled = false
	}
	...
	settings := &service.SystemSettings{
		...
		ResearchDrawingExpMode:                          req.ResearchDrawingExpMode,
		ResearchDrawingRetrievalSetting:                 req.ResearchDrawingRetrievalSetting,
		ResearchDrawingNumCandidates:                    req.ResearchDrawingNumCandidates,
		ResearchDrawingAspectRatio:                      req.ResearchDrawingAspectRatio,
		...
		ResearchDrawingMethodOptimizationDefaultEnabled: researchDrawingMethodOptimizationDefaultEnabled,
	...
	payload := dto.SystemSettings{
		...
		ResearchDrawingExpMode:                          updatedSettings.ResearchDrawingExpMode,
		ResearchDrawingRetrievalSetting:                 updatedSettings.ResearchDrawingRetrievalSetting,
		ResearchDrawingNumCandidates:                    updatedSettings.ResearchDrawingNumCandidates,
		ResearchDrawingAspectRatio:                      updatedSettings.ResearchDrawingAspectRatio,
		...
		ResearchDrawingMethodOptimizationDefaultEnabled: updatedSettings.ResearchDrawingMethodOptimizationDefaultEnabled,
