...
| `menu/` | Static menu pages mirrored from the deployment-side unified entry |
...
### Optional: PaperBanana + Unified Menu Overlay

If you want the deployment-side "科研绘图 + 统一入口" layout to live in source control too, use the optional overlay:
...
# Create runtime directories for Sub2API and PaperBanana generated files.
# PaperBanana MySQL uses the named volume paperbanana_mysql_data; no local MySQL install is needed.
mkdir -p data postgres_data redis_data
...
```powershell
docker compose -f deploy/docker-compose.local.yml -f deploy/docker-compose.paperbanana.yml down
```
...
```powershell
docker ps
docker compose -f deploy/docker-compose.local.yml -f deploy/docker-compose.paperbanana.yml logs -f
docker exec -it sub2api sh
curl -I http://research-drawing-service:8080
```
...
```text
Front-end submit
-> sub2api checks balance and creates the generation request
-> sub2api calls http://research-drawing-service:8080
-> PaperBanana returns job_id
-> front-end polls task status
-> candidate images render in the result area
```

Notes:
- `docker-compose.paperbanana.yml` is an overlay, so the default deployment path stays unchanged.
- By default the overlay builds PaperBanana from `../../../PaperBanana-fix-issue-38-refine` relative to `deploy/`.
- If your PaperBanana checkout lives elsewhere, change `PAPERBANANA_BUILD_CONTEXT` in `.env`.
- `paperbanana-mysql` runs inside Docker Compose and stores data in the named volume `paperbanana_mysql_data`.
- `paperbanana` joins the same Docker network as `sub2api` and exposes the network alias `research-drawing-service`.
- `RESEARCH_DRAWING_API_URL=http://research-drawing-service:8080` is the expected container-to-container URL.
- Keep `RESEARCH_DRAWING_SERVICE_TOKEN`, `PAPERBANANA_SERVICE_TOKEN`, and `PB_SUB2API_SERVICE_TOKEN` aligned in `.env`.
- `depends_on` waits for the MySQL healthcheck in this overlay. If an older Docker Compose implementation ignores `condition: service_healthy`, restart `paperbanana` after MySQL becomes healthy.
- The static menu content is versioned under `deploy/menu/`.
- Recommended reverse-proxy layout:
  - `https://liuloys.top/` -> Sub2API (`localhost:8080`)
  - `https://liuloys.top/menu/` -> unified menu (`localhost:8090`)
  - `https://paperbanana.liuloys.top/` -> PaperBanana (`localhost:8081`)
- See [Caddyfile](/home/project/sub2api-source/sub2api/deploy/Caddyfile:1) for the matching reverse-proxy example.
...
| `MENU_PORT` | No | `8090` | Unified menu port used by the optional PaperBanana overlay |
...
When using `docker-compose.paperbanana.yml`, also configure the `PAPERBANANA_*` variables in `.env`.
