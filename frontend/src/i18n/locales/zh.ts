export default {
  ...
  researchDrawing: {
    ...
    userHero: {
      ...
      tips: {
        first: '先参考模板风格，再填写自己的研究内容。',
        second: '每次提交消耗 1 次额度。',
        third: '生成后可在“候选结果”查看高清大图。',
      },
    },
    adminOnlyTitle: '需要管理员权限',
    adminOnlyDesc: '当前页面为系统级配置区域，请联系管理员修改参数。',
    examplesTitle: '示例图',
    examplesDesc: '这些示例用于展示生成页面中的参考效果。',
    settingsTitle: '默认生成参数',
    settingsDesc: '这里只保留必要的全局默认值，便于控制生成质量和成本。',
    examples: {
      frameworkTitle: '论文框架图',
      frameworkDesc: '',
      workflowTitle: '机制流程图',
      workflowDesc: '',
      datasetTitle: '统计图',
    ...
    input: {
      ...
      optimizeUnavailable: '方法内容优化暂未启用。',
      caption: '图注',
      generationMode: '生成模式（由管理员分配）',
      defaultMode: '基础档',
      customMode: '自定义组合（手动）',
      customModeHint: '当前为自定义组合模式：将按下方参数运行生成。',
      normalUserHint: '普通用户仅需填写方法内容与图注，系统将自动使用管理员分配的默认配置。',
      ...
      placeholders: {
        ...
        caption: '可选：图标题或图说明',
        defaultConfig: '留空按配置默认',
      },
    },
    run: {
      title: '运行生成',
      desc: '来自 PaperBanana 的后台任务流程：提交后进入等待页并通过短连接轮询状态。',
      progressTitle: '运行进度',
      progressDesc: '生成结果会自动出现在这里，点击图片可查看大图。',
      start: '运行生成',
      applyQuota: '去申请次数',
      applyQuotaHint: 'PaperBanana 源码中该按钮会跳转到申请次数区域。',
      unitPrice: '单次价格',
      quotaNeed: '预计消耗次数',
      estimatedTime: '预计本次生成耗时',
      noHistory: '3–4 分钟',
      noHistoryHint: '尚无记录，按 3–4 分钟随机估计',
      runningBanner: '后台正在生成，本页通过短连接轮询状态，不会因长时间等待而断开。',
      routesTitle: '运行接口',
      routes: {
        submit: 'POST /generate',
        wait: 'GET /generate/wait/{job_id}',
        poll: 'GET /generate/job/{job_id}（每 2 秒轮询）',
        finish: 'GET /generate/finish/{job_id}',
      },
      stepsTitle: '运行流程',
      idleStatus: '等待输入参数。PaperBanana 源码会 POST /generate 并创建后台任务。',
      previewStatus: '任务已提交到 sub2api 编排接口，由 sub2api 扣费后转交 PaperBanana 生成服务。',
      previewSubmitted: '输入已通过前端预检，正在提交 sub2api 编排接口。',
      submittedWithCharge: '任务已提交，已按 sub2api 余额扣费 {charge}。',
      submitFailed: '科研绘图任务提交失败',
      statusFailed: '科研绘图任务状态获取失败',
      imagesFailed: '生成图片加载失败',
      downloadFailed: '候选图下载失败',
      doneNotice: '科研绘图任务已完成。',
      statusTitle: '等待页状态',
      resultsTitle: '生成图片',
      saveHint: '候选图为临时生成结果，请及时下载 2K 大图保存。',
      emptyResults: '候选图为临时生成结果，请及时下载 2K 大图保存。',
      loadingImages: '正在加载生成图片…',
      download2k: '下载 2K 大图',
      generatedAt: '生成时间',
      sessionHistoryTitle: '本次会话历史图',
      sessionHistoryLimit: '最近 {count} 张',
      jobId: '任务 ID',
      candidateLabel: '候选图 #{id}',
      resultAlt: '科研绘图候选图 {id}',
      statuses: {
        idle: '未开始',
        queued: '已排队，正在生成…',
        running: '生成中… / 已进行约 {time}',
        done: '即将跳转到结果页…',
        error: '生成失败',
        missing: '任务不存在或已过期',
      },
      steps: {
        validate: '校验输入、模式和剩余次数',
        spawn: '创建后台生成任务 spawn_generation_job',
        poll: '等待页轮询任务状态',
        finish: '完成后进入候选结果页',
        export: '查看大图、下载 PNG 或保存图库',
      },
    },
    sections: {
      pipeline: '流程参数',
    ...
    labels: {
      ...
      methodOptimizationEnabled: '方法内容优化',
      methodOptimizationDefaultEnabled: '默认开启方法内容优化',
      unitPrice: '单次价格',
    },
    options: {
      enabled: '启用',
    ...
    scenesTitle: '典型场景与版式参考',
    scenes: {
      paperFramework: '适用于论文方法框架图（方法、流程、架构）的快速草图生成。',
      experimentWorkflow: '适用于实验流程图或算法步骤图，结构更清晰。',
      statisticsFigure: '适用于 SCI 风格统计图初稿，保证配色语言统一。',
    },
    tipsTitle: '使用建议',
    tips: {
      first: '1) 先参考模板写作风格再改写自己的方法内容。',
      second: '2) 每次提交消耗 1 次（按管理员配置价格购买）。',
      third: '3) 生成后可切换到“候选结果”查看大图。',
    },
    loadFailed: '科研绘图参数加载失败',
    saveFailed: '科研绘图参数保存失败',
  },
